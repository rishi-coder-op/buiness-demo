exports.handler = async (event) => {
  ...
  try {
    ...
    const limit=Math.min(Math.max(Number(count)||50,1),50), areaName=city==='All Bihar'?'Bihar':city;
    ...
    for(const term of wanted){
      ...
      const query=`[out:json][timeout:18];area["name"="${areaName}"][boundary=administrative]->.a;(nwr["name"]["office"~"${term}",i](area.a);nwr["name"]["amenity"~"${term}",i](area.a);nwr["name"]["shop"~"${term}",i](area.a);nwr["name"]["healthcare"~"${term}",i](area.a););out center tags;`;
      let data=null;
      for(const endpoint of endpoints){try{const r=await fetch(endpoint+'?data='+encodeURIComponent(query),{headers:{'User-Agent':'WebSalesAI/1.0'},signal:AbortSignal.timeout(22000)});if(r.ok){data=await r.json();break}}catch{}}
      if(!data)continue;
      for(const el of(data.elements||[])){
        ...
        seen.add(id);
        const address=[t['addr:housenumber'],t['addr:street'],t['addr:suburb'],t['addr:city'],t['addr:postcode']].filter(Boolean).join(', '),hasWebsite=Boolean(website),contactCount=[phone,email,whatsapp,social,website].filter(Boolean).length,lat=el.lat??el.center?.lat,lon=el.lon??el.center?.lon;
